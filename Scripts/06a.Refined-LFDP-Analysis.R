...
for(data_selector in seq_along(outfiles)){
  ...
  ########################################################
  ### Figure 2 - there are a variety of plots below; need to decide which to include
  ########################################################
  ...
  if(data_selector==length(outfiles)){
    write.csv(plot_level_summary_table, "Processed_data/plot_level_summary_table.csv", row.names = F)
  }

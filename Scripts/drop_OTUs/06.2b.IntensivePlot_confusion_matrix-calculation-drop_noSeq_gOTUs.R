########################################################
### COMPUTE CONFUSION MATRIX INDICES WITH THE INTENSIVE SUB-SAMPLE PLOTS, OTUs NOT IN SEQ DATA REMOVED
########################################################

library(phyloseq)

### Get names of intensive plot sample points phyloseq object
label <- names(readRDS("Processed_data/PR_eDNA-for-analysis_intenplot_2025-04-01.RData"))

for(gdat in seq_along(label)){

  message(paste("working on bioinformatic filtering", label[gdat]), ": ", gdat)
  ...
  saveRDS(list(conf_stats_obs=conf_stats_obs, 
          ...
          file=paste0("Processed_data/Conf_matrix_output-Intensive-", 
                 label[gdat], 
                 "-20250406.RDA"))
}
